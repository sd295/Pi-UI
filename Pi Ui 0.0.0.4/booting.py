...
for i in range(10):  # Adjust the range based on the number of images
    ...
    resized_image = pygame.transform.scale(image, (695, new_height))
...
while frame_index < total_frames:
    ...
    screen.blit(image, (0, (960 - new_height) // 2))  # Center the image vertically

    # Draw the text "pi" at the center of the screen
    text = font.render("pi os 0.0.0.4*(4)*", True, (255, 255, 255))
    text_rect = text.get_rect(center=(330, 480))  # Center the text
    screen.blit(text, text_rect)

    # Display the loading dots
    loading_dots = "." * (frame_index % 4)
    loading_text = font.render(loading_dots, True, (255, 255, 255))
    loading_text_rect = loading_text.get_rect(center=(330, 596))  # Center the loading dots
...
time.sleep(3.45)  # Wait for the Python interpreter to start
